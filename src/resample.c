...
{
   const rsdatad *cpy = (const rsdatad *)handle;
   rsdatad *hp = (rsdatad *)malloc(sizeof(rsdatad));
   ...
   hp->Imp = (double *)malloc(hp->Nwing * sizeof(double));
   memcpy(hp->Imp, cpy->Imp, hp->Nwing * sizeof(double));
   hp->ImpD = (double *)malloc(hp->Nwing * sizeof(double));
   memcpy(hp->ImpD, cpy->ImpD, hp->Nwing * sizeof(double));

   hp->Xoff = cpy->Xoff;
   hp->XSize = cpy->XSize;
   hp->X = (double *)malloc((hp->XSize + hp->Xoff) * sizeof(double));
   memcpy(hp->X, cpy->X, (hp->XSize + hp->Xoff) * sizeof(double));
   hp->Xp = cpy->Xp;
   hp->Xread = cpy->Xread;
   hp->YSize = cpy->YSize;
   hp->Y = (double *)malloc(hp->YSize * sizeof(double));
   memcpy(hp->Y, cpy->Y, hp->YSize * sizeof(double));
   hp->Yp = cpy->Yp;
   hp->Time = cpy->Time;
...
void *resample_opend(int highQuality, double minFactor, double maxFactor)
{
   double *Imp64;
   double Rolloff, Beta;
   rsdatad *hp;
   ...
   hp = (rsdatad *)malloc(sizeof(rsdatad));
...
{
   const rsdatad *hp = (const rsdatad *)handle;
   return hp->Xoff;
}
...
int resample_processd(void   *handle,
                     ...
                     int     outBufferLen)
{
   rsdatad *hp = (rsdatad *)handle;
   double  *Imp = hp->Imp;
   double  *ImpD = hp->ImpD;
   double  LpScl = hp->LpScl;
   UWORD  Nwing = hp->Nwing;
   BOOL interpFilt = FALSE; /* TRUE means interpolate filter coeffs */
   int outSampleCount;
   UWORD Nout, Ncreep, Nreuse;
   ...
   *inBufferUsed = 0;
   ...
   for(;;) {
      ...
      *inBufferUsed += len;
      ...
      /* Resample stuff in input buffer */
      if (factor >= 1) {      /* SrcUp() is faster if we can use it */
         Nout = lrsSrcUpd(hp->X, hp->Y, factor, &hp->Time, Nx,
                         Nwing, LpScl, Imp, ImpD, interpFilt);
      }
      else {
         Nout = lrsSrcUDd(hp->X, hp->Y, factor, &hp->Time, Nx,
                         Nwing, LpScl, Imp, ImpD, interpFilt);
      }
